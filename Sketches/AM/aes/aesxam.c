...
void cycles(volatile unsigned long *rtn)
{
#if defined(_MSC_VER)
...
{   static unsigned long a[2], mt = 1, count = 4;
    ...
    if(mt) { mt = 0; cycles((unsigned long *)a); }
...
{   FILE            *fin = 0, *fout = 0;
    ...
    cp = enckey;   // this is a pointer to the hexadecimal key digits
    ...
    if(!(fin = fopen(infile, "rb")))   // try to open the input file
    {
        printf("The input file: %s could not be opened\n", infile);
        err = -5; goto exit;
    }

    if(!(fout = fopen(outfile, "wb")))  // try to open the output file
    {
...
{   FILE            *fin = 0, *fout = 0;
    ...
    cp = deckey;   // this is a pointer to the hexadecimal key digits
    i = 0;          // this is a count for the input digits processed
...
int main () {
	aes_enc("test.plain","test.enc","12345678901234567890123456789012");
	aes_dec("test.enc","test.dec","12345678901234567890123456789012");
}
